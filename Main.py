...
class WindowsAddBattle(QMainWindow, Ui_Frame_add_battle):
    ...
    def re_b(self):
        try:
            ...
            QMessageBox.information(self, "测试-编队-错误", e.message, QMessageBox.Yes)
        except Exception as e:
            QMessageBox.information(self, "测试-编队-错误", str(e), QMessageBox.Yes)
...
class WindowsMain(QMainWindow, Ui_Frame_main):
    ...
    def __init__(self):
        ...
        self.setupUi(self)
        self.setWindowTitle('测试 Beta' + str(VERSION) + " 本软件永久免费, 任意售卖均未经许可")
        self.menu_event()
    ...
    def menu_event(self):
        ...
        action_about_author.triggered.connect(lambda: QMessageBox.information(self, '测试-关于',
                                                                              '版本:Beta-' + str(VERSION)
                                                                              + '\n作者: Simon菌'
                                                                              + '\nQQ群:560271993'
        ...
        try:
            ...
            for user in data['list']:
                ...
                for ship in user['ships']:
                    ship_path = 'icon/photo/' + str(int(init_data.ship_cid_wu[ship['shipCid']]['shipIndex'])) + ".png"
...
class OtherFunction:
    ...
    def show_mine(windows):
        try:
            QMessageBox.information(windows, '测试-我的',
    ...
    def get_log(windows):
        log.get_log()
        QMessageBox.information(windows, "测试", "已将日志输出至桌面!", QMessageBox.Yes)
    ...
    def dismantle_equipment(item):
        if item == 0:
            rep = QMessageBox.question(windows_main, '测试', '将会自动分解三星以下的装备\n请提前上锁重要装备\n如:绿声呐,绿投弹\n是否继续?', QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
        ...
        if 'version' in gameData.mine:
            if gameData.mine['version'] > VERSION:  # 版本过期
                ...
                if data != '':
                    speak += '\n更新日志:' + data
                speak += '\n是否去下载最新版本?'
                reply = QMessageBox.question(windows_main, '测试', speak, QMessageBox.Yes | QMessageBox.No,
                                             QMessageBox.Yes)
            ...
            QMessageBox.critical(windows_main, '测试', "无法连接更新服务器!", QMessageBox.Yes,
                                 QMessageBox.Yes)
    ...
    def change_name(item):
        if item == 0:
            rep = QMessageBox.question(windows_main, '测试', '名称反和谐是利用游戏自带改名\n将动物园改为正常名称\n根据船只多少可能花费数分钟\n是否继续?', QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            if rep == QMessageBox.Yes:
